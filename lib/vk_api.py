...
class response:
    ...
    def get_response(self):
        if 'response' not in self.http_response:
            return False
...
class list:
    ...
    def exec(self):
        ...
        if first_response is False:
            print(self.first_request.get_response().get_error()['error_msg'])
            return
        ...
        while offset < count:
            offset += int(self.first_request.params['count'])
